...
{
    internal class ImpHashGen
    {
        private static readonly Dictionary<UInt16, String> oleaut32_ordnames = new Dictionary<UInt16, String>
        {
            { 2, "SysAllocString" },
            { 3, "SysReAllocString" },
            { 4, "SysAllocStringLen" },
        ...
        private static readonly Dictionary<UInt16, String> ws2_32_ordnames = new Dictionary<UInt16, String>
        {
            { 1, "accept" },
            { 2, "bind" },
            { 3, "closesocket" },
        ...
        internal static CryptoHash GetImpHash(ImageImportDescriptor[] imports)
        {
            if (imports is null)
                return null;

            var str_table = new List<String>();
            var exts = new String[] { ".ocx", ".sys", ".dll" };

            foreach (var imp in imports)
            {
                var libname = Path.GetFileName(imp.Name).ToLower();

                if (exts.Contains(Path.GetExtension(libname)))
                    libname = Path.GetFileNameWithoutExtension(libname);

                foreach (var func in imp.LookupTable)
                {
                    String funcname = null;

                    if (String.IsNullOrEmpty(func.Name))
                    {
                        if ((libname == "ws2_32") || (libname == "wsock32"))
                        {
                            if (ws2_32_ordnames.ContainsKey((UInt16)func.Ordinal))
                                funcname = String.Format("{0}.{1}", libname, ws2_32_ordnames[(UInt16)func.Ordinal].ToLower());
                        }
                        else if (libname == "oleaut32")
                        {
                            if (oleaut32_ordnames.ContainsKey((UInt16)func.Ordinal))
                                funcname = String.Format("{0}.{1}", libname, oleaut32_ordnames[(UInt16)func.Ordinal].ToLower());
                        }

                        if (String.IsNullOrEmpty(funcname))
                            funcname = String.Format("{0}.ord{1}", libname, func.Ordinal);
                    }
                    else
                    {
                        funcname = String.Format("{0}.{1}", libname, func.Name.ToLower());
                    }

                    if (!String.IsNullOrEmpty(funcname))
                        str_table.Add(funcname);
                }
            }

            return new CryptoHash(
                Encoding.ASCII.GetBytes(String.Join(",", str_table)),
                CryptoHash.CryptoHashType.MD5);
        }
    }
}
